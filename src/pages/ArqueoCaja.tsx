...
const ArqueoCaja = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Crear traslado automáticamente después del arqueo
      const { data: turnoData } = await supabase
        .from("turnos")
        .select("caja_id, empleado_id")
        .eq("id", aperturaActiva.turno_id)
        .single();
      ...
      const { data: trasladoData, error: trasladoError } = await supabase
        ...
        .insert({
          ...
          turno_id: aperturaActiva.turno_id,
          empleado_envia_id: turnoData?.empleado_id,
          caja_origen_id: turnoData?.caja_id,
          caja_destino_id: cajaPrincipal?.id,
          monto: montoContado,
          estado: "pendiente",
      ...
      const { error: recepcionError } = await supabase
        ...
        .insert({
          ...
          empleado_recibe_id: turnoData?.empleado_id,
          monto_recibido: 0,
          diferencia: 0,
          fecha_hora: null,
          comentario: null,
      ...
      toast({
        ...
        description: "El turno ha sido cerrado y el traslado ha sido registrado",
      });

      navigate("/traslado-efectivo");
    } catch (error: any) {
      toast({
        title: "Error",
        description: error.message,
